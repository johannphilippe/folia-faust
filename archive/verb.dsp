...
with {
	...
	with {
		deltime = reflectionL(index) : _* ma.SR, !;
		delamp = reflectionL(index) : !, _;
	};
};
delR(sig) = sum(n, nreflectionsR, apply(n+1))
with {
	apply(index) = sig : de.delay(MAXDEL_R, deltime) : *(delamp) 
	with {
		deltime = reflectionR(index) : _* ma.SR, !;
...
with {
    ...
    //delays = (101, 199, 307, 743, 1009, 2341, 3847, 4999);
    //delays = (101, 199, 743, 1009, 3847, 4999,  13681, 19997);
    delays = (2287, 2347, 3079, 3301, 3779, 3853, 4673, 4751);
...
mixdel(mix, verblevel, s1, s2) = (s1 * imix) + (vl * mix), (s2 * imix) + (vr * mix)
with {
	imix = 1 - mix;
	l = s1 : delL;
	r = s2 : delR;
    verb = s1+(l*mix), s2+(r*mix) <: rev : *(verblevel), *(verblevel);
    //verb = s1+ (l*mix), s2 + (r * mix) : free : *(verblevel), *(verblevel);
    //verb = s1, s2 : re.stereo_freeverb(0.4, 0.4, 0.6, 0) : *(verblevel), *(verblevel);
    vl = verb : ba.selectn(2, 0) : +(l); //ba.take(1, verb);
    vr = verb : ba.selectn(2, 1) : +(r); //ba.take(2, verb);
};

mix = hslider("mix", 0.3, 0, 1, 0.001);
amp = hslider("amp", 0.7, 0, 1, 0.001);
longverb = hslider("longverb", 0.25, 0, 1, 0.001);
process = _,_ : mixdel(mix, longverb) : *(amp), *(amp);
